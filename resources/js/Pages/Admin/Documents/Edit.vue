...
watch(() => [form.source_text, JSON.stringify(form.fields)], () => { dirty.value = true; temporaryPreview.value = false; });
  ...
  <AdminLayout :title="`Editar documento · ${document.public_code}`" eyebrow="Documentos">
    ...
    <section class="admin-content document-editor">
      ...
      <div class="editor-grid">
        <section class="panel preview-panel" :class="{ 'mobile-hidden': activeTab !== 'preview' }"><div class="panel-head"><div><h2>{{ temporaryPreview ? 'Vista previa de cambios' : 'Vista previa actual' }}</h2><small v-if="temporaryPreview" class="preview-badge">VISTA PREVIA - AÚN NO GUARDADO</small></div><div><button class="row-action" type="button" @click="openPreview">Ampliar</button><a v-if="!temporaryPreview" class="row-action" :href="route('admin.documents.download', { document: document.id, version: 'issued' })">Descargar actual</a></div></div><iframe :src="previewUrl" title="PDF vigente" /></section>
